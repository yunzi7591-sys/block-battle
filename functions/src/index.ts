...
// ============================================================================
// Types (mirror of client-side RoomData, server-authoritative subset)
// ============================================================================

interface RoomData {
    status: "waiting" | "playing" | "finished";
    player1: { uid: string; name: string; rate: number };
    player2?: { uid: string; name: string; rate: number };
    isRanked: boolean;
    isFinished?: boolean;
    winner?: string;  // UID of the winner
    loser?: string;   // UID of the loser
    gameStartTime?: number; // serverTimestamp() set at game start
    isRatingCalculated?: boolean; // Idempotency flag
}
...
export const onGameFinished = onValueUpdated(
    {
        ref: "/rooms/{roomId}",
    ...
    async (event) => {
        ...
        const after = event.data.after.val() as RoomData | null;
        ...
        if (!participants.has(after.winner) || !participants.has(after.loser)) {
            console.warn(
                `[CF] Room ${roomId}: winner=${after.winner} / loser=${after.loser} ` +
        ...
        const txResult = await roomRef.transaction((currentData: RoomData | null) => {
            if (!currentData) {
        ...
        if (typeof gameStartTime === "number" && gameStartTime > 0) {
            ...
            if (gameDurationMs < MIN_GAME_DURATION_MS) {
                console.warn(
                    `[CF] Room ${roomId}: Suspicious fast game ` +
            ...
            console.warn(
                `[CF] Room ${roomId}: gameStartTime missing or invalid (${gameStartTime}). ` +
        ...
        const winnerCurrentRating: number =
            (winnerDoc.exists && typeof winnerDoc.data()?.rating === "number")
                ? winnerDoc.data()!.rating
                : DEFAULT_RATING;

        const loserCurrentRating: number =
            (loserDoc.exists && typeof loserDoc.data()?.rating === "number")
        ...
        batch.set(
            firestore.doc(`users/${after.winner}`),
            {
        ...
        batch.set(
            firestore.doc(`users/${after.loser}`),
            {
